"""
Client-side proxy for teledyneT3AFG200 that communicates with the TCP server
Provides the exact same interface as teledyneT3AFG200 but sends commands over TCP
Uses persistent connection for better performance

This client is intentionally identical in shape to client_keysight33622A.py:
the underlying instrument method names are the same, so the over-the-wire
JSON-RPC payloads (instrument='function_gen', method=<name>, args, kwargs)
are interchangeable. The server just needs to be pointed at the new
teledyneT3AFG200 backend instead of keysight33622A.
"""
...
class ClientTeledyneT3AFG200:
    """
    Client proxy for teledyneT3AFG200 that communicates with a TCP server
    ...
    def __init__(self, ipAddress: str = "", server_host: str = "localhost",
        ...
        :param server_host: Host where the Teledyne server is running
        :param server_port: Port where the Teledyne server is listening
    ...
    def connect(self):
        ...
        with self._lock:
            ...
            try:
                ...
                print(f"Connected to Teledyne server at {self.server_host}:{self.server_port}")
                return True

            except Exception as e:
                print(f"Failed to connect to Teledyne server: {e}")
                self._connected = False
                if self._socket:
                    try:
                        self._socket.close()
    ...
    # All the teledyneT3AFG200 methods
    def init(self):
    ...
    def set_high_level(self, channel: int, high: float):
        ...
        return self._send_request_with_retry('set_high_level', channel, high)

    def set_low_level(self, channel: int, low: float):
        """Set the low voltage level (V)"""
        return self._send_request_with_retry('set_low_level', channel, low)

    def set_duty_cycle(self, channel: int, duty: float):
        """Set the duty cycle (%)"""
        return self._send_request_with_retry('set_duty_cycle', channel, duty)
...
teledyneT3AFG200 = ClientTeledyneT3AFG200


if __name__ == '__main__':
    # Test the client
    print("Testing Teledyne T3AFG200 client with persistent connection...")

    client = ClientTeledyneT3AFG200()
